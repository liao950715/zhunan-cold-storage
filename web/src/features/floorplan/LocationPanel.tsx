...
export default function LocationPanel({ locationId, onAction }: { locationId: number; onAction?: (action: "inbound" | "outbound" | "transfer", detail: LocationDetail) => void }) {
  ...
  return (
    <div className="space-y-3 text-sm">
      <div>
        <h3 className="text-lg font-bold">{d.location.code}</h3>
      ...
      {d.currentProduct ? (
        <div className="rounded bg-green-50 border border-green-200 p-3">
          <p className="font-medium text-green-900">{d.currentProduct.name}</p>
        ...
        <div className="rounded bg-slate-50 border border-slate-200 p-3 text-slate-500">
          空儲位{d.location.defaultCapacity !== null && `（預設容量 ${d.location.defaultCapacity}）`}
        </div>
      )}
      {d.lines.length > 0 && (
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        <button className="btn" onClick={() => onAction?.("inbound", d)}>從此儲位入庫</button>
        {d.occupied > 0 && (
          <>
            <button className="btn" onClick={() => onAction?.("outbound", d)}>從此儲位出庫</button>
            <button className="btn" onClick={() => onAction?.("transfer", d)}>搬移</button>
          </>
        )}
      </div>
    </div>
